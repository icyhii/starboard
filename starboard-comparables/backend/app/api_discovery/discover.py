...
def normalize_field(field_name: str) -> str:
    ...
    try:
        ...
        normalized = response.choices[0].message['content'].strip() if hasattr(response.choices[0], 'message') else response.choices[0].text.strip()
        if normalized:
            return normalized
    except Exception:
        pass
...
def detect_fields_of_interest(fields: List[Dict[str, str]]) -> List[Dict[str, str]]:
    return [f for f in fields if f["normalized"] in ["property_id", "zoning", "square_feet", "construction_year", "address"]]
